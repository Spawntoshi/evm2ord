...
- **No hosting? No problem.** The reference platform can host a whole multi-image collection — upload each piece, it builds the per-token metadata, and each token's `image` **upgrades independently to its own Bitcoin ordinal** as it's inscribed. Batch-inscribe the entire set in one commit+reveal, non-custodially from your own BTC wallet. (Or skip the host and go fully on-chain via `setInscriptions` — SPEC §8b.)
- **Enforced royalties — real ERC-721C (default at deploy).** Because royalties *fund the inscriptions*, launches default to a **real ERC-721C** contract (LimitBreak Creator Token Standard): it exposes `ICreatorToken` + ERC-2981 and auto-registers the canonical on-chain transfer validator, so **OpenSea recognizes it as enforced** — tokens only sell through royalty-honoring operators, while wallet-to-wallet transfers by the holder stay free. Validated on Robinhood Chain mainnet (validator `0x721C002B…` is live there). Plain ERC-2981 (unenforced) remains an opt-out. See SPEC §2 for the contract + the OZ-v4.9.6 pin and other gotchas.
- **True on-chain provenance (opt-in at inscribe).** The render engine can double as the ordinals **parent inscription**: every child is inscribed with the `parent` tag *and* its reveal spends the parent sat, so `ordinals.com/r/children/<parent>` lists the whole collection — provable on-chain, no off-chain manifest. Optional **cover image** on the parent. Set at inscription time (can't be retrofitted). See SPEC §6b.
- **Server-free metadata (image on Bitcoin).** Store each token's inscription id on-chain and flip `setOnchainMetadata(true)` — `tokenURI` then returns a self-contained `data:application/json;base64,…` whose image is the token's **Bitcoin inscription**. No host, no IPFS, no Arweave; progressive fallback to baseURI; owner-swappable ord gateway. See SPEC §8b.
- **Auto-refresh on OpenSea — ERC-4906.** Metadata changes on-chain (image → ordinal, base → IPFS, on-chain flip), and the contract emits the standard `MetadataUpdate` / `BatchMetadataUpdate` events (and reports `supportsInterface(0x49064906)`), so **OpenSea re-reads `tokenURI` automatically** — no per-item refreshing, no support ticket. Built in at deploy; can't be retrofitted to an immutable contract. See SPEC §8c.
